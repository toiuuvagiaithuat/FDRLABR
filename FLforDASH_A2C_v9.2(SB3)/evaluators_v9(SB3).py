...
# from flearn.users.useravgPPO import UserAVGPPO
...
class A2CEvaluator(BaseEvaluator):
    def evaluate(self, file_name, model_file):
        test_agent = A2C.load(model_file, env = self.env)
...
if __name__ == '__main__':
    ...
    # eval.evaluate('test_constant_B' + str(config.env_config["max_buffer"]) + data)

    # eval = SmoothEvaluator(data)
    # eval.evaluate('smooth1600_s0.9_B' + str(config.env_config["max_buffer"]) + data)
    # # #
    ...
    #
    dir_name = "./resultsTuned_nstep5/local20/realUpDown40_c100_cpr10_globiter500_locepi20_seed1/"
    print(dir_name)

    model_file = "bestmodel.zip"
    
    start_t = time.time()
    eval = A2CEvaluator(data)
    eval.evaluate(dir_name + model_file, dir_name + model_file)

    end_t = time.time()
    list2csv(dir_name + "eval_time", [end_t - start_t])
