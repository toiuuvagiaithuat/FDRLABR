...
import numpy as np
import math
# from data.get_down_size import video_list_collector
from utils.utils import VideoListCollector
...
class Env(gym.Env):
    ...
    BUFFER_THRESHOLD = config.env_config["max_buffer"]  # Max buffer
    M_IN_K = 1000
    VIDEO_CHUNK_LEN = 4  # Length of each chunk, in second

    def __init__(self, user_id, env_config: Dict[str, Any], bitrate_list=None, istrain=None):
        # evn_params['']
        # user_id, video_trace='data/video_list_bunny8k', bw='env_data/bw/synthetic100u.pickle'):
        self.user_id = user_id
        self.total_user = config.system_config["total_users"]
        self.env_config = env_config

        if (env_config["log_qoe"]==True):
        ...
        obs_space = dict(
            ...
            next_chunk_size= gym.spaces.Box(low=0, high=float("inf"), shape=(self.SEGMENT_SPACE,), dtype=np.float32),
            ...
            last_down_quality=gym.spaces.Box(low=0, high=self.QUALITY_SPACE, shape=(1,), dtype=np.int8),
    ...
    def reset(self):
        ...
        if self.istrain: # train
            bw_idx = np.random.choice(len(self.bitrate_list))
        ...
        self.state = dict(
            ...
            next_chunk_size= np.zeros(self.SEGMENT_SPACE, ),
            buffer_size=np.array([0]),
            percentage_remain_video_chunks=np.array([1]),
            last_down_quality=np.array([self.DEFAULT_QUALITY]),
    ...
    #         flat_state = np.append(flat_state, self.state[key])
    ...
    #         flat_state = np.append(flat_state, state[key])
    #     return flat_state
    ...
    def step(self, action):
        ...
        self.state = dict(
            ...
            last_down_quality=np.array([self.download_segment[down_id]]),
            delay=self.delay_net,
        )
        if self.play_id == self.CHUNK_TIL_VIDEO_END - 1:
...
if __name__ == "__main__":
    np.random.seed(42)
    test = np.asarray(pickle.load(open('bw/LTE_test.pickle', 'rb')))
    env = Env(0, config.env_config)
    env.reset()
    done = False
    while not done:
