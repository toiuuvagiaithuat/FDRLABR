...
class FedAvg():
    def __init__(self):
        ...
        dqn = DQN("MultiInputPolicy", env=init_env, **config.DQN_params)
        self.model = dqn.q_net # global model
        ...
        for uid in range(self.total_users):
            ...
            # env = Monitor(Env(uid, config.env_config, bw), info_keywords = info_keywords)
            # env.bitrate_list = bw

            # self.rl_agent = DQN(env.get_state_num(), env.get_action_num())
            # model = DQN("MlpPolicy", env)

            # user = UserAVG(user_id=uid, rlagent=model, env)
            user = UserAVG(user_id=uid, bw = bw)
    ...
    def train(self):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            list2csv(dir_name + config.log_config["data_filename"], [avg_reward])
            ...
            if best_reward < avg_reward:
                ...
                self.users[0].rlagent.save(f"{dir_name}_bestmodel")
