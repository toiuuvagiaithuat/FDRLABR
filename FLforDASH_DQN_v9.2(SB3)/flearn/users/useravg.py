...
class UserAVG():
    def __init__(self, user_id, bw=None):
        ...
        self.rlagent = DQN("MultiInputPolicy", env = self.env, **config.DQN_params)
    ...
    def get_parameters(self):
        for param in self.rlagent.q_net.parameters():
            param.detach()
        return self.rlagent.q_net.parameters()

    # set parameter for client model to a new weight
    def set_parameters(self, new_model):
        for new_param, local_param in zip(new_model.parameters(), self.rlagent.q_net.parameters()):
            local_param.data = new_param.data.clone()

    def train(self):
        dir_name = config.log_config["dir_name"]

        self.rlagent.learn(total_timesteps=60*self.local_epochs, reset_num_timesteps=False, tb_log_name=f"{dir_name}_logs")
